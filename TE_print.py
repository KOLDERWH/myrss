...
if __name__ == '__main__':
    ...
    def TESpider():
        html = sessions.get(domain_url + "/printedition", headers=headers)
    ...
    def getArticle(url_article):
        ...
        # article=bs.find('article').find('div').find('div')
        if article is None:
    ...
    def makeRss(rssItems):
        rss = PyRSS2Gen.RSS2(
            ...
            items=rssItems)
        rss.write_xml(open('./xml/TheEconomist_Print_'+str(datetime.datetime.now().month) + "-"+str(datetime.datetime.now().day) + '.xml', "w", encoding='utf-8'), encoding='utf-8')
        pass
